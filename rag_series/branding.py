...
OWNER_TAGLINE = "ai engineer| RaG Expert| Building RAG workflow that works in production"
...
def apply_streamlit_branding(
    ...
    *,
    ...
    """
    Apply consistent Streamlit branding.

    IMPORTANT: Call this before the first Streamlit UI call (after imports).
    """
...
def fastapi_description_md(brand: Brand = DEFAULT_BRAND) -> str:
    return (
        f"Production-ready RAG tutorial (Hassan Khan RAG Series).  \n\n"
        f"Maintainer: **{brand.name}**  \n"
        f"LinkedIn: {brand.linkedin}  \n"
        f"Email: `{brand.email}`"
    )
